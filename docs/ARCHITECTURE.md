...
The public `POST /api/rsvp/search` route trims and lowercases the submitted term, then performs an exact normalized match against invited-member last names. Each successful result discloses only the household `displayName` and private `rsvpUrl`; invite codes are not returned. Search returns at most 10 results. If more than 10 recoverable households match, it returns an empty result list with `tooManyMatches: true` instead of partially disclosing the matches. The route is limited to 5 attempts per normalized search term per hour and 20 attempts per source IP per hour, in addition to API Gateway throttling of 2 requests per second with a burst of 5. It obtains each private link through the existing recoverable-invite-code path, which decrypts the stored invite-code ciphertext with the KMS-backed protector and verifies the resulting code before constructing the URL.
